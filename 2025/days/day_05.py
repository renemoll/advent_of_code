...
Part 2:
    ...
    This can be done by generating a set of all IDs in the ranges, but I assume the amount of IDs is too large for decent speed/memory.
    So we need to reduce the number of ranges to non-overlapping ranges, and then sum their lengths.
"""

from .utilities import merge_overlapping_ranges
...
def _part2(parsed_input) -> int:
    ...
    return sum(r.stop - r.start for r in merge_overlapping_ranges(ranges))
